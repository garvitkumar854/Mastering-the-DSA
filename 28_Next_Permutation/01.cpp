...
similarly 3<4,4<5, but {2>5} called pivot in Permutation      
Characterstic of pivot: A[i] < A[i+1] {if Backward Travels}
Step 1: Find the pivot
Step 2: Find the righmost element > pivot element
Step 3: Swap pivot with rightmost element.
Step 4: Reverse elements from (pivot+1) to n-1.

If in a Situation that pivot not exits so we will arrange array in ascending order {Question Says}
...
vector<int> nextPermutation(vector<int> &arr){          // {TC = O(n), SC = O(1)}
    ...
    // Find Rightmost Element which is > pivot.
    for(int i = n-1; i>pivot; i--){
        if(arr[i] > arr[pivot]){
            swap(arr[i], arr[pivot]);     // swap the element with pivot
            break;
        }
    }

    // Reverse the Part after pivot to n-1.
